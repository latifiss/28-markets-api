...
function initRealtimeWebsocketServer(server, opts) {
  ...
  if (wss) return wss;
  const path =
    (_a = opts === null || opts === void 0 ? void 0 : opts.path) !== null &&
    _a !== void 0
      ? _a
      : '/ws';
  const allowedOrigins =
    (_b = opts === null || opts === void 0 ? void 0 : opts.allowedOrigins) !==
      null && _b !== void 0
      ? _b
      : [];
  ...
  wss.on('connection', (ws, req) => {
    var _a, _b;
    const url = new URL(
      (_a = req.url) !== null && _a !== void 0 ? _a : path,
      'http://localhost',
    );
    const topicsParam =
      (_b = url.searchParams.get('topics')) !== null && _b !== void 0 ? _b : '';
    ...
    ws.on('message', (raw) => {
      ...
      try {
        ...
        ws.send(
          JSON.stringify({ event: 'error', message: 'Invalid JSON message' }),
        );
        return;
      }
      if (msg.type === 'ping') {
        ws.send(
          JSON.stringify({ event: 'pong', now: new Date().toISOString() }),
        );
        return;
      }
