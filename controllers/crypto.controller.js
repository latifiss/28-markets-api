...
exports.bulkUpsertCryptos = exports.getCoinHistoryStats = exports.deleteCoinHistory = exports.getAllCoinHistories = exports.getCoinHistory = exports.createCoinHistory = exports.bulkUpdateCoinGainers = exports.getTopGainers = exports.getCoinGainer = exports.getAllCoinGainers = exports.getComprehensiveData = exports.getTopLosers = exports.syncCoinLosers = exports.bulkUpdateCoinLosers = exports.deleteCoinLoser = exports.updateCoinLoser = exports.getCoinLoser = exports.getAllCoinLosers = exports.createCoinLoser = exports.createCoinGainer = exports.updateCryptoPrice = exports.addCryptoHistory = exports.getCryptoHistory = exports.deleteCrypto = exports.updateCryptoBySymbol = exports.getCryptoBySymbol = exports.getAllCryptos = exports.updateCryptoById = exports.getCryptoById = exports.createCrypto = void 0;
...
const invalidateCryptoCache = async (symbol = null) => {
...
const createCrypto = async (req, res) => {
    try {
        const { id, symbol, name, image, current_price, market_cap, market_cap_rank, fully_diluted_valuation, total_volume, high_24h, low_24h, price_change_24h, price_change_percentage_24h, market_cap_change_24h, market_cap_change_percentage_24h, } = req.body;
        ...
        const existingCrypto = await crypto_model_1.Crypto.findOne({
        ...
        const newCrypto = new crypto_model_1.Crypto({
            id,
        ...
        (0, ws_1.publishCryptoUpdate)(savedCrypto.symbol, savedCrypto);
        res.status(201).json({
            success: true,
            code: 201,
...
const getCryptoById = async (req, res) => {
    try {
        ...
        const crypto = await crypto_model_1.Crypto.findOne({
            $or: [
                { id: id },
...
const updateCryptoById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (current_price !== undefined &&
            ...
            updateOperations.$set.price_change_24h = parseFloat(price_change_24h.toFixed(4));
            updateOperations.$set.price_change_percentage_24h = parseFloat(price_change_percentage_24h.toFixed(4));
        ...
        const updatedCrypto = await crypto_model_1.Crypto.findOneAndUpdate({ id: crypto.id }, updateOperations, { new: true, runValidators: true });
        await invalidateCryptoCache(updatedCrypto.symbol);
        (0, ws_1.publishCryptoUpdate)(updatedCrypto.symbol, updatedCrypto);
        res.status(200).json({
            success: true,
            code: 200,
...
exports.updateCryptoById = updateCryptoById;
const getAllCryptos = async (req, res) => {
    try {
        ...
        if (minPrice > 0 || maxPrice < Infinity) {
            query.current_price = {};
            if (minPrice > 0)
                query.current_price.$gte = minPrice;
            if (maxPrice < Infinity)
                query.current_price.$lte = maxPrice;
        }
        if (minMarketCap > 0 || maxMarketCap < Infinity) {
            query.market_cap = {};
            if (minMarketCap > 0)
                query.market_cap.$gte = minMarketCap;
            if (maxMarketCap < Infinity)
                query.market_cap.$lte = maxMarketCap;
        }
        const skip = (page - 1) * limit;
        const total = await crypto_model_1.Crypto.countDocuments(query);
        const sort = {};
        sort[sortBy] = sortOrder === 'desc' ? -1 : 1;
        const cryptos = await crypto_model_1.Crypto
            .find(query)
            .sort(sort)
            .skip(skip)
            .limit(limit)
...
exports.getCryptoBySymbol = getCryptoBySymbol;
const updateCryptoBySymbol = async (req, res) => {
    var _a;
    try {
        ...
        const updatedCrypto = await crypto_model_1.Crypto.findOneAndUpdate({ symbol: symbol }, updateOperations, { new: true, runValidators: true });
        await invalidateCryptoCache(symbol);
        (0, ws_1.publishCryptoUpdate)((_a = updatedCrypto.symbol) !== null && _a !== void 0 ? _a : symbol, updatedCrypto);
        res.status(200).json({
            success: true,
            code: 200,
            message: 'Crypto updated successfully',
...
exports.updateCryptoBySymbol = updateCryptoBySymbol;
const deleteCrypto = async (req, res) => {
    try {
        ...
        const deletedCrypto = await crypto_model_1.Crypto.findOneAndDelete({
            symbol: symbol,
        });
        if (!deletedCrypto) {
            res.status(404).json({
        ...
        await crypto_model_1.CoinGainers.findOneAndDelete({
            symbol: symbol,
        });
        await crypto_model_1.CoinLosers.findOneAndDelete({
            symbol: symbol,
        });
        await invalidateCryptoCache(symbol);
        res.status(200).json({
...
const getCryptoHistory = async (req, res) => {
    try {
        ...
        const crypto = await crypto_model_1.Crypto
            .findOne({
            $or: [
...
exports.getCryptoHistory = getCryptoHistory;
const addCryptoHistory = async (req, res) => {
    try {
        ...
        const updatedCrypto = await crypto_model_1.Crypto.findOneAndUpdate({ id: crypto.id }, {
            $push: {
                price_history: {
                    $each: [newPriceEntry],
                    $position: 0,
...
exports.addCryptoHistory = addCryptoHistory;
const updateCryptoPrice = async (req, res) => {
    var _a;
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
        ...
        const updatedCrypto = await crypto_model_1.Crypto.findOneAndUpdate({ symbol: symbol }, updateOperations, { new: true });
        await invalidateCryptoCache(symbol);
        (0, ws_1.publishCryptoUpdate)((_a = updatedCrypto.symbol) !== null && _a !== void 0 ? _a : symbol, updatedCrypto);
        res.status(200).json({
            success: true,
...
exports.updateCryptoPrice = updateCryptoPrice;
const createCoinGainer = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const missingFields = requiredFields.filter((field) => !coinGainerData[field]);
        if (missingFields.length > 0) {
            res.status(400).json({
                success: false,
                code: 400,
        ...
        const existingGainer = await crypto_model_1.CoinGainers.findOne({
            $or: [
        ...
        const syncResult = await (0, crypto_model_1.syncWithCrypto)();
...
exports.createCoinGainer = createCoinGainer;
const createCoinLoser = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const missingFields = requiredFields.filter((field) => !coinLoserData[field]);
        if (missingFields.length > 0) {
            res.status(400).json({
                success: false,
                code: 400,
        ...
        const existingLoser = await crypto_model_1.CoinLosers.findOne({
            symbol: symbolValue,
        });
        if (existingLoser) {
            const updatedLoser = await crypto_model_1.CoinLosers.findOneAndUpdate({ symbol: symbolValue }, {
                ...coinLoserData,
                symbol: symbolValue,
                last_updated: new Date(),
        ...
        const newCoinLoser = new crypto_model_1.CoinLosers({
            ...coinLoserData,
            symbol: symbolValue,
            last_updated: new Date(),
        });
...
exports.createCoinLoser = createCoinLoser;
const getAllCoinLosers = async (req, res) => {
    try {
        if (req.rateLimit && req.rateLimit.remaining === 0) {
            res.status(429).json({
                success: false,
        ...
        const skip = (page - 1) * limit;
        const total = await crypto_model_1.CoinLosers.countDocuments(query);
        const sort = {};
        sort[sortBy] = sortOrder === 'desc' ? -1 : 1;
        const coinLosers = await crypto_model_1.CoinLosers
            .find(query)
            .sort(sort)
            .skip(skip)
            .limit(limit)
...
exports.getAllCoinLosers = getAllCoinLosers;
const getCoinLoser = async (req, res) => {
    try {
        const symbolParam = req.params.symbol;
        const symbol = ensureString(symbolParam).toUpperCase();
        if (!symbol) {
        ...
        const coinLoser = await crypto_model_1.CoinLosers.findOne({
            symbol: symbol,
        });
        if (!coinLoser) {
            res.status(404).json({
...
exports.getCoinLoser = getCoinLoser;
const updateCoinLoser = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const updatedLoser = await crypto_model_1.CoinLosers.findOneAndUpdate({ symbol: symbol }, {
            ...updateData,
            last_updated: new Date(),
        }, { new: true, runValidators: true });
        if (!updatedLoser) {
...
exports.updateCoinLoser = updateCoinLoser;
const deleteCoinLoser = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const deletedLoser = await crypto_model_1.CoinLosers.findOneAndDelete({
            symbol: symbol,
        });
        if (!deletedLoser) {
            res.status(404).json({
...
exports.deleteCoinLoser = deleteCoinLoser;
const bulkUpdateCoinLosers = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        coinLosersData.forEach((entry, index) => {
            const errors = [];
            if (!entry.symbol)
                errors.push('symbol');
            if (!entry.name)
                errors.push('name');
            if (entry.percentage_change_24h === undefined)
                errors.push('percentage_change_24h');
            if (!entry.current_price)
                errors.push('current_price');
            if (errors.length > 0) {
        ...
        const result = await crypto_model_1.CoinLosers.bulkWrite(operations);
...
exports.bulkUpdateCoinLosers = bulkUpdateCoinLosers;
const syncCoinLosers = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
...
exports.syncCoinLosers = syncCoinLosers;
const getTopLosers = async (req, res) => {
    try {
        ...
        const topLosers = await crypto_model_1.CoinLosers
            .find()
...
const getComprehensiveData = async (req, res) => {
    try {
        ...
        const comprehensiveData = await (0, crypto_model_1.getComprehensiveCoinData)(symbol);
...
exports.getComprehensiveData = getComprehensiveData;
const getAllCoinGainers = async (req, res) => {
    try {
        if (req.rateLimit && req.rateLimit.remaining === 0) {
            res.status(429).json({
                success: false,
        ...
        const skip = (page - 1) * limit;
        const total = await crypto_model_1.CoinGainers.countDocuments(query);
        const sort = {};
        sort[sortBy] = sortOrder === 'desc' ? -1 : 1;
        const coinGainers = await crypto_model_1.CoinGainers
            .find(query)
            .sort(sort)
            .skip(skip)
            .limit(limit)
...
exports.getAllCoinGainers = getAllCoinGainers;
const getCoinGainer = async (req, res) => {
    try {
        const symbolParam = req.params.symbol;
        const symbol = ensureString(symbolParam).toUpperCase();
        if (!symbol) {
        ...
        const coinGainer = await crypto_model_1.CoinGainers.findOne({
            symbol: symbol,
        });
        if (!coinGainer) {
            res.status(404).json({
...
exports.getCoinGainer = getCoinGainer;
const getTopGainers = async (req, res) => {
    try {
        ...
        const topGainers = await crypto_model_1.CoinGainers
            .find()
            .sort({ percentage_change_24h: -1 })
            .limit(limit)
            .lean();
...
const bulkUpdateCoinGainers = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const validationErrors = [];
        coinGainersData.forEach((entry, index) => {
            const errors = [];
            if (!entry.symbol)
                errors.push('symbol');
            if (!entry.name)
        ...
        const result = await crypto_model_1.CoinGainers.bulkWrite(operations);
        await invalidateCryptoCache();
        res.status(200).json({
            success: true,
            code: 200,
...
exports.bulkUpdateCoinGainers = bulkUpdateCoinGainers;
const invalidateCoinHistoryCache = async (symbol = null) => {
    await deleteCacheByPattern('coinhistory:*');
    if (symbol) {
        await deleteCacheByPattern(`coinhistory:${symbol}:*`);
    }
};
const createCoinHistory = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        market_data.prices.forEach((pricePoint, index) => {
        ...
        const formattedPrices = market_data.prices.map(([timestamp, price]) => ({
            timestamp,
            price,
        }));
        const existingHistory = await crypto_model_1.CoinHistory.findOne({
            symbol: symbol.toUpperCase(),
            timeframe,
            vs_currency: vs_currency || 'usd',
        });
        if (existingHistory) {
            const updatedHistory = await crypto_model_1.CoinHistory.findOneAndUpdate({
                symbol: symbol.toUpperCase(),
                timeframe,
                vs_currency: vs_currency || 'usd',
            }, {
        ...
        const newCoinHistory = new crypto_model_1.CoinHistory({
            symbol: symbol.toUpperCase(),
            name,
            market_data: { prices: formattedPrices },
            timeframe,
...
const getCoinHistory = async (req, res) => {
    try {
        ...
        const coinHistory = await crypto_model_1.CoinHistory.findOne({
            symbol,
            timeframe,
            vs_currency,
        });
        ...
        if (startDate > 0 || endDate < Date.now()) {
            prices = prices.filter((point) => point.timestamp >= startDate && point.timestamp <= endDate);
        }
...
const getAllCoinHistories = async (req, res) => {
    try {
        if (req.rateLimit && req.rateLimit.remaining === 0) {
            res.status(429).json({
                success: false,
        ...
        const skip = (page - 1) * limit;
        const total = await crypto_model_1.CoinHistory.countDocuments(query);
        const histories = await crypto_model_1.CoinHistory.find(query)
...
exports.getAllCoinHistories = getAllCoinHistories;
const deleteCoinHistory = async (req, res) => {
    try {
        const apiKey = req.headers['x-api-key'];
        if (!apiKey || apiKey !== process.env.API_KEY) {
            res.status(401).json({
        ...
        const deletedHistory = await crypto_model_1.CoinHistory.findByIdAndDelete(id);
        if (!deletedHistory) {
            res.status(404).json({
                success: false,
                code: 404,
...
exports.deleteCoinHistory = deleteCoinHistory;
const getCoinHistoryStats = async (req, res) => {
    try {
        const symbolParam = req.params.symbol;
        const symbol = ensureString(symbolParam).toUpperCase();
        const timeframe = ensureString(req.query.timeframe) || '30';
...
exports.getCoinHistoryStats = getCoinHistoryStats;
const bulkUpsertCryptos = async (req, res) => {
    var _a, _b;
    try {
        ...
        for (const cryptoData of cryptos) {
            try {
                const existingCrypto = await crypto_model_1.Crypto.findOne({ id: cryptoData.id });
                if (existingCrypto) {
                    const updated = await crypto_model_1.Crypto.findOneAndUpdate({ id: cryptoData.id }, {
                        ...cryptoData,
                    ...
                    (0, ws_1.publishCryptoUpdate)((_a = updated === null || updated === void 0 ? void 0 : updated.symbol) !== null && _a !== void 0 ? _a : cryptoData.symbol, updated !== null && updated !== void 0 ? updated : cryptoData);
                }
                else {
                    const newCrypto = new crypto_model_1.Crypto({
                        ...cryptoData,
                    ...
                    (0, ws_1.publishCryptoUpdate)((_b = newCrypto.symbol) !== null && _b !== void 0 ? _b : cryptoData.symbol, newCrypto);
                }
        ...
        (0, ws_1.broadcast)('crypto', 'bulk_upsert_completed', results);
    }
    catch (error) {
...
exports.bulkUpsertCryptos = bulkUpsertCryptos;
